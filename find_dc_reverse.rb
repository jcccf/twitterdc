...
class FindDirectedClosure
  ...
  def iterate(curr,time)
    ...
    if @citers[curr]
      @seen.each do |s|
        if @citers[s][curr] && @citers[s][curr] < time # The Reverse, i.e. someone @ messaged this person first
          found += 1
        end
      end
      if found > 0
...
if __FILE__ == $0
  ...
  DC_dir_rev = Constants::DC_dir + '_rev'
  
  Dir.mkdir(DC_dir_rev) unless File.directory? DC_dir_rev
  ...
  File.open(celebrity_file, "r") do |cfile|
    while line = cfile.gets
      ...
      output_file = DC_dir_rev+"/"+Constants::DC_pref + a + Constants::DC_suff
